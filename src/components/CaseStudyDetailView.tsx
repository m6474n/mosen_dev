...
import { ArrowLeft, CheckCircle2, ChevronRight, Hash, Database, ExternalLink } from 'lucide-react';
import { useData } from '../context/DataContext';
import DefaultPageLayout, { Container } from './DefaultPageLayout';
import DynamicPageHeader from './DynamicPageHeader';
import ReusableCard from './ReusableCard';
import InteractiveScreenshotMockup from './InteractiveScreenshotMockup';

interface CaseStudyViewProps {
  currentHash: string;
}

export default function CaseStudyDetailView({ currentHash }: CaseStudyViewProps) {
  const { caseStudies } = useData();
  const [activeScreenshotIdx, setActiveScreenshotIdx] = useState(0);

  // Parse project ID if any
  const isDetail = currentHash.startsWith('#/case-study/');
  const projectId = isDetail ? currentHash.replace('#/case-study/', '') : '';
  const currentProject = caseStudies.find(p => p.id === projectId);


  // Safely reset active tab when shifting between separate case studies
  useEffect(() => {
    setActiveScreenshotIdx(0);
  }, [projectId]);

  // Fallback if detail path is active but project is not found
  if (isDetail && !currentProject) {
    return (
      <DefaultPageLayout>
        <Container className="items-center justify-center py-20 text-center">
          ...
          <a href="#/case-studies" className="px-6 py-3 bg-neutral-950 text-white text-xs font-bold tracking-wider uppercase">
  ...
  // Render individual Case Study detail page
  if (currentProject) {
    return (
      <DefaultPageLayout>
        <Container>
          ...
          <ReusableCard hoverable={false} variant="muted" className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 md:p-8 border-neutral-100 mb-4">
            {currentProject.results.map((res, id) => (
              <div key={id} className="flex flex-col border-r last:border-0 border-neutral-200 pr-4">
  ...
  // Render comprehensive Case Studies dynamic list
  return (
    <DefaultPageLayout>
      <Container>
        <DynamicPageHeader
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-neutral-100">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {project.tech.slice(0, 3).map((t, idx) => (
                      <span key={idx} className="text-[9px] font-bold text-neutral-400 tracking-wider bg-neutral-100 px-2 py-0.5 uppercase">
                        {t}
                      </span>
                    ))}
                  </div>
